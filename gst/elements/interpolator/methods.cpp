...
GstMemory *resize(const GstMapInfo &map_info, GstVideoInfo *video_info, int width, int height) {
    ...
    cv::cvtColor(resized, color_converted_to_rgb, cv::ColorConversionCodes::COLOR_RGBA2RGB);

    size_t buffer_size = color_converted_to_rgb.total() * color_converted_to_rgb.elemSize();
    unsigned char *data = new unsigned char[buffer_size];
    memcpy(static_cast<void *>(data), static_cast<void *>(color_converted_to_rgb.data), buffer_size);
...
GstFlowReturn opencv_resize(GstInterpolator *interpolator, GstBuffer *in_buffer, GstBuffer *out_buffer) {
    ...
    GstMapInfo info;
    gst_memory_map(resized_to_destination_size, &info, GST_MAP_READ);
    gst_memory_unmap(resized_to_destination_size, &info);
